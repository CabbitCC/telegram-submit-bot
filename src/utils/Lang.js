...
class Lang {
  ...
  getViaInfo (message) {
    ...
    } else if (msgInfo.type == 'user'){
      text = this.get('via_user', this.getUser(message.from)); 
    }
    //限制在10
  ...
  getFromText (message) {
    ...
    text += '\n' + this.get('sub_from', this.getUser(message.from))
    return text;
  };
  /**
   * 得到 来源保留：保留/匿名 这一行
   ...
   */
  getMoreHelp () {
    return this.get('admin_morehelp', {command: '/ahelp'});
  };
  /**
   * 获取审稿群通用头部
   * @param  {[type]} message [description]
