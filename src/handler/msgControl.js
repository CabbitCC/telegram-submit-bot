...
export default
{
  ...
  subAsk (message) {
    ...
    if (!message.forward_date) {
      ...
      inline_keyboard[0].push({ text: lang.get('no'), callback_data: vars.SUB_REAL });
    }
  ...
  async receiveMessage (message, receive, params = {}) {
    ...
    let receive_url = 'https://t.me/TradeUndress/'+ resp.message_id + '?comment=1';
    // 向用户发送稿件过审信息
    await bot.sendMessage(message.chat.id, lang.get('sub_finish_tip'), { 
      reply_to_message_id, 
      reply_markup: {
        inline_keyboard: [[{text: lang.get('sub_receive_url'), url: receive_url}]]
      }
    })
